...
import json
import requests
# library that works is "kafka-python" 
from kafka import KafkaConsumer, KafkaProducer
from kafka.oauth import AbstractTokenProvider
import sys
from utils.utils import get_mode, get_slash # internal function to get system slash ('\' for windows '/' for linux). It should be replaced by os.path.join()
from config.config import get_config
from json import dumps, loads

_config = get_config() # script to get config values from a specific file  MUST BE CHANGED
    ...
    def __init__(self):
        ...
        try:
            self.producer = KafkaProducer(bootstrap_servers=BOOTSTRAP_SERVERS,
                                          ...
                                          ssl_cafile=sys.path[1] + get_slash() + 'env_values' + get_slash() + KAFKA_CERT,
    ...
    def __init__(self):
        ...
        try:
            self.consumer = KafkaConsumer(bootstrap_servers=BOOTSTRAP_SERVERS,
                                          ...
                                          sasl_mechanism='OAUTHBEARER',
                                          sasl_oauth_token_provider=TokenProvider(),
                                          ssl_cafile=os.path.join(os.path.join(sys.path[1], 'env_values'), KAFKA_CERT),
